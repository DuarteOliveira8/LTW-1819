...
let showRules = new XMLHttpRequest();
...
showRules.onreadystatechange = function() {
  if (this.readyState === 4 && this.status === 200) {
    let response = JSON.parse(this.responseText);
    if (!response.success) {
      posts.innerHTML = `<h1>There are no rules. Run!</h1>`;
    }
    for(let i = 0; i < response.data.length; i++) {
      //TODO
    }
...
// showPosts.open("POST", "/api/home/recent-posts", true);
...
//       showPosts.open("POST", "/api/channel/" + channelId, true);
